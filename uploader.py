...
class MotionUploader:
    ...
    def upload_image(self, image_file_path):
        """Upload a image to the specified folder. Then optionally send an email and optionally delete the local file."""
        folder_id = self._get_folder_id(self.folder)

        media = MediaFileUpload(image_file_path, mimetype='image/png')
        response = self.drive_service.files().insert(media_body=media, body={'title':os.path.basename(image_file_path), 'parents':[{u'id': folder_id}]}).execute()
        #print response
        image_link = response['alternateLink']
...
if __name__ == '__main__':
    try:
        ...
        if option.lower() == 'image':
            MotionUploader(cfg_path).upload_image(vid_path)
        else:
